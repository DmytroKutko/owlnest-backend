...
**Status:** Implemented for private profile avatars. Managed post attachments remain deferred.
...
The existing `post_media.url` contract is unchanged. It still stores untrusted absolute HTTPS references, and managed post image/video attachment is a later slice.

## Authenticated avatar flow

1. `POST /api/v1/media/uploads` accepts the implemented `AVATAR` purpose, reserves immutable metadata, and returns a short-lived presigned `PUT` URL.
...
4. `PUT /api/v1/profile/me/avatar` atomically activates owned `READY` avatar media and marks the previous avatar for later cleanup.
5. A profile or post author projection returns `/api/v1/media/{mediaId}/delivery`. Authenticated `POST` to that URL rechecks the current active avatar association and returns a short-lived presigned private `GET` URL.
...
- Purpose-specific allowlists and limits are enforced in HTTP validation, domain code, and PostgreSQL. Avatars accept JPEG, PNG, or WebP up to 10 MiB.
...
- `AWAITING_UPLOAD -> READY -> ACTIVE` is the successful avatar path.
- Expiry, cancellation, replacement, and removal transition to `DELETION_PENDING`; physical deletion later produces `DELETED`.
- Confirmation is idempotent after a successful confirmation. Avatar replacement uses a stable lock order and one PostgreSQL transaction.
...
- managed post attachment or migration of existing `post_media.url` rows;
